...
#[get("/?<remoteurl..>")]
fn getpdf(remoteurl: Option<Form<RemoteURL>>, screens: State<AvailableScreens>) -> Result<Response<'static>, status::Custom<String>> {
...
fn get_screen(screens: &AvailableScreens) -> Result<usize, String> {
    ...
    if no_screens {
      Err("No available screens".to_string())
    } else {
      lock[screen] = true;
      Ok(screen)
    }
...
fn main() {
    rocket::ignite()
        .mount("/", routes![getpdf])
        .manage(AvailableScreens { screens: Mutex::new(vec![false;4])})
        .launch();
}